...
export default function Http() {
    ...
    function handleSyncSuccess() {
        ...
        request.open("GET", "http://localhost:8080/api/success", false);
        request.send(null);
    ...
    function handleAsyncSuccess() {
        let request = new XMLHttpRequest();
        request.open("GET", "http://localhost:8080/api/success");
        request.send();
    ...
    function handleNotFound() {
        ...
        xhr.open("GET", "http://localhost:8080/api/notFound", true);
    ...
    function handleInterServer() {
        let xhr = new XMLHttpRequest();
        xhr.open("GET", "http://localhost:8080/api/error", true);
        xhr.onload = function (e) {
            if (xhr.readyState === 4) {
                if (xhr.status === 200) {
                    console.log(xhr.responseText);
    ...
    function handleTimeOut() {
        ...
        xhr.open(
            "GET",
            "http://127.0.0.1:4523/m1/671786-0-default/pet/1",
            true
        );
        xhr.timeout = 10;
        xhr.onload = function () {
    ...
    async function handleFetch404() {
        const response = await fetch("http://localhost:8080/api/notFound");
        const data = await response.json();
    ...
    return (
        <div className="info">
            ...
            <button onClick={handleSyncSuccess} className="info--button">
                origin sync req success
            </button>
            <button onClick={handleAsyncSuccess} className="info--button">
                origin async req success
            </button>
            <button onClick={handleNotFound} className="info--button">
                req 404
            </button>
            <button onClick={handleInterServer} className="info--button">
                req 500
            </button>
            {/* <button onClick={handleTimeOut} className="info--button">
                req timeout
            </button> */}
            <button onClick={handleFetch} className="info--button">
                handle Fetch
            </button>
            <button onClick={handleFetch404} className="info--button">
                Fetch 404
            </button>
            <button onClick={handleFetch500} className="info--button">
                Fetch 500
            </button>
